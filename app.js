// app.js (completo e com Apostas 100% funcionais)
...
// ===== Config
const ADMIN_EMAILS = [];           // se quiser admins “fixos” por e-mail
const BET_COST = 2;                // custo fixo por aposta
const MIN_WALLET = 6;              // saldo mínimo padrão
...
function showTab(tab){
  ...
  // NÃO redireciona mais para "home" em nenhum fluxo de formulário
...
function listenAdmin(user){
  ...
  state.listeners.admin = onSnapshot(adminsRef, (snap)=>{
    ...
    const byDoc  = d?.isAdmin === true || d?.active === true;
...
function updateAuthUI(){
  ...
  // aposta só logado: desabilita campos se não logado
...
async function ensureWalletInit(uid){
  ...
  try{
    await runTransaction(db, async (tx)=>{
      ...
      if(!snap.exists()){
        tx.set(refWal, { points: MIN_WALLET, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
      }else{
        const cur = snap.data() || {};
        const curPts = typeof cur.points==="number" ? cur.points : 0;
        const newPts = Math.max(curPts, MIN_WALLET);
        if(newPts !== curPts){
    ...
    // fallback
    try{
      await setDoc(refWal, { points: MIN_WALLET, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge:true });
...
function listenWallet(uid){
  ...
  state.listeners.wallet = onSnapshot(doc(db,"wallets",uid),(snap)=>{
    ...
    state.wallet = Math.max(pts, MIN_WALLET);
...
function probVED(m){
  ...
  const bets = state.betsAll ? state.betsAll.filter(b=> b.matchId===m.id) : [];
...
// odds simples a partir de probabilidade implícita (com margem)
function computeOddsForPick(m, pick){
  const p = probVED(m);
  const prob = (pick==="A" ? p.A : pick==="B" ? p.D : p.E) / 100;
  // margem casa ~ 8% + clamps
  const raw = 1 / Math.max(0.05, Math.min(0.95, prob));
  const withMargin = raw * 0.92;
  return Math.max(1.3, Math.min(6.0, Number(withMargin.toFixed(2))));
}
...
// (extra) carregar todas bets (para probabilidade por “mercado”)
async function loadAllBetsOnce(){
  try{
    const qs = await getDocs(collection(db,"bets"));
    state.betsAll = qs.docs.map(d=>({id:d.id, ...d.data()}));
  }catch(e){
    state.betsAll = [];
  }
}
...
// ===== Home (próximas pendentes do dia mais próximo)
function renderHome(){
  const mapP=Object.fromEntries(state.players.map(p=>[p.id,p.name]));
  const pending = state.matches.filter(m=> !m.result || m.result==="postponed")
    .filter(m=> !!m.date)
    .slice()
    .sort((a,b)=>parseLocalDate(a.date)-parseLocalDate(b.date));

  // agrupa por dia, pega o dia mais próximo ao “agora” que tenha partidas pendentes
  let dayCandidates = {};
  pending.forEach(m=>{
    const d = parseLocalDate(m.date); if(!d) return;
    const key = `${d.getFullYear()}-${fmt2(d.getMonth()+1)}-${fmt2(d.getDate())}`;
    (dayCandidates[key] ||= []).push(m);
  });
  const keys = Object.keys(dayCandidates).sort((ka,kb)=> new Date(ka) - new Date(kb));
  const firstKey = keys[0];
  const pick = firstKey ? dayCandidates[firstKey].slice(0,4) : [];
  ...
  const table=`
    <table>
      ...
      <tbody>${rows||"<tr><td colspan='3'>Sem partidas pendentes agendadas.</td></tr>"}</tbody>
    </table>
    <div class="home-next-extra">
      <p>Mostrando até <b>4</b> partidas pendentes do dia mais próximo com jogos agendados.</p>
      <p>Edite/defina datas e resultados na aba <b>Partidas</b>.</p>
    </div>
  `;
...
// ===== Bets
function listenBets(){
  // bets do usuário (pra tabela e settlement do próprio)
  ...
  // também carregamos todas bets (uma vez) para probabilidade de “mercado”
  loadAllBetsOnce();
}
...
function renderBetsSelect(){
  ...
  const upcoming = state.matches.filter(m=>{
    if(m.result && m.result!=="postponed") return false;
    const d = parseLocalDate(m.date);
    // permite apostar se não tem data ou se ainda não começou
...
function renderBets(){
  ...
  tbody.innerHTML = state.bets.map(b=>{
    ...
    const extra = b.odds ? ` — cota: ${b.odds}x` : "";
    return `<tr>
      <td>${name}</td>
      <td>${etapa}</td>
      <td>${b.pick==="A"?"Vitória A": b.pick==="B"?"Vitória B":"Empate"}${extra}</td>
      <td>${b.status||"pendente"}</td>
    </tr>`;
  }).join("");
}

// transação para registrar aposta + debitar carteira
async function placeBetTransaction({ uid, matchId, pick }){
  const walletRef = doc(db, "wallets", uid);
  const matchRef  = doc(db, "matches", matchId);
  const betRef    = doc(collection(db,"bets")); // gera id

  await runTransaction(db, async (tx)=>{
    // carteira
    const wSnap = await tx.get(walletRef);
    let points = MIN_WALLET;
    if(wSnap.exists() && typeof wSnap.data().points === "number"){
      points = wSnap.data().points;
    }else{
      tx.set(walletRef, { points: MIN_WALLET, createdAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge:true });
      points = MIN_WALLET;
    }
    if(points < BET_COST) throw new Error("Saldo insuficiente.");

    // partida
    const mSnap = await tx.get(matchRef);
    if(!mSnap.exists()) throw new Error("Partida não encontrada.");
    const m = { id: matchId, ...mSnap.data() };
    if(m.result && m.result!=="postponed") throw new Error("Partida já concluída.");
    if(m.date){
      const d = parseLocalDate(m.date);
      if(d && d.getTime() <= Date.now()) throw new Error("Não é possível apostar após o início da partida.");
    }

    // odds calculadas no momento da aposta
    const odds = computeOddsForPick(m, pick);
    const stake = BET_COST;
    const potentialReturn = Number((odds * stake).toFixed(2));

    // cria aposta
    tx.set(betRef, {
      uid, matchId, pick,
      stake, odds, potentialReturn,
      status: "pendente",
      createdAt: serverTimestamp()
    });
...
function bindBetForm(){
  const form = $("#bet-form");
  const submitBtn = $("#bet-form button[type='submit']");

  // redundância: clique no botão também chama submit (se algum navegador travar 'submit')
  submitBtn?.addEventListener("click", (e)=>{
    // não deixa “pular” de aba por algum comportamento estranho
    e.preventDefault();
    e.stopPropagation();
    form?.requestSubmit?.();
  });
  ...
  form?.addEventListener("submit", async (e)=>{
    ...
    try{
      ...
      if(!matchId || !pick) return alert("Selecione a partida e o palpite.");
      await placeBetTransaction({ uid: state.user.uid, matchId, pick });
      alert("Aposta registrada! (-2 pontos)");
      // limpa campos, mantém na aba de apostas
      e.target.reset();
      renderBetsSelect(); // recarrega lista de partidas apostáveis
    }catch(err){
      console.error("bet add", err);
      alert(err?.message || "Erro ao registrar aposta.");
    }
  });
}

// (opcional) quitação automática das apostas do usuário quando partidas terminarem
async function settleBetsIfFinished(){
  ...
  for(const b of state.bets){
    ...
    if(ok){
      // crédito do retorno (stake já foi debitado no ato da aposta)
      const walletRef = doc(db, "wallets", state.user.uid);
      const snap = await getDoc(walletRef);
      const cur = (snap.exists() && typeof snap.data().points==="number") ? snap.data().points : MIN_WALLET;
      const gain = b.potentialReturn ?? (b.odds ? Number((b.odds * (b.stake||BET_COST)).toFixed(2)) : (BET_COST+2));
      write.set(walletRef, { points: Math.max(MIN_WALLET, cur + gain), updatedAt: serverTimestamp() }, { merge:true });
    }
    changed = true;
  }
